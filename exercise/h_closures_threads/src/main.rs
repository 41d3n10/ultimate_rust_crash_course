...
fn main() {
    ...
    let child_a = thread::spawn(move || {
        for msg in ra {
            println!("Thread Child A: Received {}", msg);
        }
        println!("Thread Child A: Exiting.");
    });
    let child_b = thread::spawn(move || {
        for msg in rb {
            println!("Thread Child B: Received {}", msg);
        }
        println!("Thread Child B: Exiting.");
    });
    for i in 1..101 {
        pause_ms(100);
        s.send(i).unwrap();
    }
    println!("Main Thread: Dropping s.");
    drop(s);
